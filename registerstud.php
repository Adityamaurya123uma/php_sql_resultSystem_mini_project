...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        $emailquery = "select * from student_main where stud_email='$email'";
    ...
    ?>
            ...
            if ($emailcount === 0) {
                $insertquery = "insert into student_main(stud_name, stud_course, stud_contact, stud_gender, stud_email, stud_pswd, stud_pic, stud_status) VALUES ('$name','$course','$mobile','$gender','$email','$password','$photo','$status')";
    ...
    <table cellpadding="2" width="20%" align="center" cellspacing="2" border="2">
        ...
        $records = mysqli_query($con, "select * from student_main"); // fetch data from database

        while ($data = mysqli_fetch_array($records)) {
        ?>
            <tr>
                <td><?php echo $data['student_id']; ?></td>
                <td><?php echo $data['stud_name']; ?></td>
                <td><?php echo $data['stud_gender']; ?></td>
                <td><?php echo $data['stud_course']; ?></td>
                <td><?php echo $data['stud_email']; ?></td>
                <td><?php echo $data['stud_contact']; ?></td>
                <td><?php echo $data['stud_pswd']; ?></td>
                <td><?php echo $data['stud_status']; ?></td>
            </tr>
        <?php
        }
        ?>
